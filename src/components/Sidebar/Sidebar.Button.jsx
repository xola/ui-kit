...
export const SidebarButton = ({ icon: Icon, label, appearance = "plain", className, ...rest }) => {
    ...
    return (
        <button
            ...
            {...rest}
        >
            <div className={cn("shrink-0", isSolid ? "p-0.5" : "p-1.5", !showIcons && "hidden")}>
                <Icon className={cn("shrink-0", isSolid ? "h-5 w-5" : "h-4 w-4")} />
            </div>
...
SidebarButton.propTypes = {
    ...
    // "solid" is the filled, card-like treatment that lifts one action out of the list. It
    // carries a larger icon to match.
