...
// Each menu is an independent tippy instance, so nothing stops two being open at once, and an
// instance cannot always hide itself: @tippyjs/react forces trigger "manual" whenever `visible` is
// set. Every visible menu registers here and the one opening evicts the rest.
...
// Resizing the rail moves a trigger without a scroll or window resize, the only events popper
// repositions on, so an open menu would sit at its pre-drag x. Tippy also counts the resize handle
// as inside the interactive border, so leaving the link for the handle never hides it.
