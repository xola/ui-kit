...
export const SidebarLink = ({
    ...
    if (isSubMenuItem) {
        return (
            <button
                ...
                {...rest}
            >
                <Dot className={cn("mr-3 shrink-0", isActive ? "bg-white" : "bg-gray")} />

                <span className={cn("px-1 text-left", classNames?.text)}>{children}</span>
            </button>
        );
    }
    ...
    return (
        ...
        >
            ...
            <div className={cn("shrink-0", !showIcons && "hidden")}>
                {Icon && <Icon className={cn("h-5 w-5 shrink-0", showIcons && showText && "mr-3")} />}
            </div>
            ...
            {/* The default chevron promises a flyout, so it is gated on `hasSubmenu`. A consumer
                `info` node is opt-in content (a badge, a portal target) and renders regardless. */}
            {info ??
                (hasSubmenu && <ChevronRightIcon className={cn("ml-auto h-3 w-3 shrink-0", !showText && "hidden")} />)}
        </button>
    );
};
...
SidebarLink.propTypes = {
    ...
    classNames: PropTypes.shape({ button: PropTypes.string, text: PropTypes.string }),
};
