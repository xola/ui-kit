...
async function buildConfig() {
    const { extend: extendedConfig } = config.theme;
    ...
    return `
    {
        ...
        theme: {
            ...
            fontSize: ${JSON.stringify(config.theme.fontSize, null, 2)},

            borderRadius: ${JSON.stringify(config.theme.borderRadius, null, 2)},

            extend: {
                fontFamily: ${JSON.stringify(extendedConfig.fontFamily, null, 2)},

                lineHeight: ${JSON.stringify(extendedConfig.lineHeight, null, 2)},

                letterSpacing: ${JSON.stringify(extendedConfig.letterSpacing, null, 2)},
...
function header() {
    const now = new Date();
    return `/**
    * Auto-generated by ${path.basename(__filename)}. Do not change it manually!
    */
`;
}
...
async function writeFile(data, filename) {
    ...
    const formattedConfig = await prettier.format(data, { ...pkg.prettier, parser: "babel" });

    fs.writeFileSync(filepath, formattedConfig, "utf8");
    console.log(`${fileExists ? "Updated" : "Created"} ${filename}`);
}

async function generateCommonJS() {
    ...
    module.exports = ${await buildConfig()};
`;

    await writeFile(commonJSConfig, "tailwind.config.cjs");
    console.log();
}

async function generateESM() {
    const esmConfig = `
    ${header()}
...
`;
    ...
    await writeFile(esmConfig, "tailwind.config.js");
}
